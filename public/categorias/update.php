...
if (!Utilidades::comprobarIdCategoria($idCat, $idCategorias)) {
    header("Location:index.php");
    die();
}
$miCategoria = Categorias::read($idCat);
if (isset($_POST['btn'])) {
    ...
    if ($nombre != $miCategoria[0]->nombre) {
    ...
    if ($errores) {
        header("Location:{$_SERVER['PHP_SELF']}?id={$idCat}");
        die();
    }
...
<body style="background-color:blanchedalmond">
    <div class="container p-8 mx-auto">
        ...
        <h3 class="text-2xl text-center mt-4">Editar Categoria</h3>
        <div class="w-3/4 mx-auto p-6 rounded-xl bg-gray-400">
            <form method="POST" action="<?php echo $_SERVER['PHP_SELF'] . "?id=" . $idCat ?>" enctype="multipart/form-data">
                <div class="mb-6">
                    <label for="nombre" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Nombre</label>
                    <input type="text" name="nombre" id="nombre" value="<?php echo $miCategoria[0]->nombre ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <?php
