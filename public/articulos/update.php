...
if (isset($_POST['btn'])) {
    ...
    if (is_uploaded_file($_FILES['imagen']['tmp_name'])) {
        if (Utilidades::comprobarImagen($_FILES['imagen']['type'], $_FILES['imagen']['size'])) {
            ...
            if (!move_uploaded_file($_FILES['imagen']['tmp_name'], $ruta . $imagen)) {
                ...
                if (basename($articulo[0]->imagen) != 'default.png') {
                    unlink("./../" . $articulo[0]->imagen);
                }
            }
        } else {
            $errores = true;
    ...
    if ($errores) {
        header("Location:{$_SERVER['PHP_SELF']}?id={$idArticulo}");
        die();
    }
    (new Articulos)->setNombre($nombre)
        ...
        ->setImagen($imagen)
        ->update($idArticulo);
    $_SESSION['mensaje'] = "Articulo actualizado con exito";
    header("Location:index.php");
    die();
}
?>
            ...
            <form method="POST" action="<?php echo $_SERVER['PHP_SELF'] . "?id=" . $idArticulo ?>" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <div class="relative">
                                <input type="checkbox" name="disponible" value="j" class="sr-only peer" <?php echo ($articulo[0]->disponible == "SI") ? "checked" : ""; ?>>
                ...
                <div class="mb-6">
                    ...
                    <select name="categoria" id="categoria" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        foreach ($categorias as $categoria) {
                            $seleccionado =  ($articulo[0]->category_id == $categoria->id) ? "selected" : "";
                            echo <<<TXT
                                <option {$seleccionado} value="{$categoria->id}">{$categoria->nombre}</option>
                                TXT;
                        }
                        ?>
                    </select>
                ...
                <div class="mb-6">
                    <div class="flex w-full">
                        ...
                        <div class="w-1/2">
                            <img src="../<?php echo $articulo[0]->imagen ?>" class="h-72 rounded w-full object-cover border-4 border-black" id="img">
                        </div>
                    </div>
